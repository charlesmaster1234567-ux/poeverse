...
So never look at me nor smile
For in my heart rudeness is concentrated I'm a bad idea
My heart has gained greatness and happiness
Never follow me nor call for me when I walk a mile
No bore down under my feet even if you are interested
Never plead piteously, asking for my forgiveness
I won't forgive you, will fire you in each your trail
Your beauty is a poisonous flower indeed
You are guilty in my mind though sinless
You cannot cross my heart again as a river Nile`
  },
  poem6: {
    title: "Not Well?",
    category: "Sonnet",
...
This's not my full heart, something out of me
World is not ordinary, 's new and dull day
...
And this is my soul`
  ...
  poem13: {
    ...
    number: "✦",
    time: "2 min",
    content: `[dreams & actions: 10-18 Nov 2024]

Don't you see? Can use googles
...
Ukakumbuka mawu anga aja, "palibenso"
...
let stats = JSON.parse(localStorage.getItem('stats')) || { poems: 0, time: 0, visits: 0, readPoems: [] };
...
document.addEventListener('keydown', (e) => {
  const themes = {
    '1': { bg: '#4b0082', text: '#fff' },
    '2': { bg: '#708090', text: '#ffff00' },
    '3': { bg: '#006400', text: '#ffc0cb' },
    '4': { bg: '#ff69b4', text: '#4b0082' },
    '5': { bg: '#ffd700', text: '#000' },
    '6': { bg: '#00ced1', text: '#00008b' },
    '7': { bg: '#ff4500', text: '#008000' },
    '8': { bg: '#dc143c', text: '#000080' },
    '9': { bg: '#1a1a2e', text: '#f0f0f5' }
  };
  
  if (themes[e.key]) {
    document.body.style.backgroundColor = themes[e.key].bg;
    document.body.style.color = themes[e.key].text;
  }
});
...
function closePoemModal() {
  ...
  if (currentSpeech) {
    ...
    currentSpeech = null;
    speakerBtn.querySelector('.action-icon').textContent = '🔊';
...
function updateFavoriteButton() {
  ...
  favoriteBtn.querySelector('.action-icon').textContent = isFavorite ? '❤️' : '🤍';
  favoriteBtn.classList.toggle('active', isFavorite);
}

favoriteBtn.addEventListener('click', () => {
  const index = favorites.indexOf(currentPoemId);
  if (index > -1) {
...
speakerBtn.addEventListener('click', () => {
  ...
  const poem = poemsData[currentPoemId];
  if (!poem) return;
  ...
  speakerBtn.querySelector('.action-icon').textContent = '⏹️';
...
document.querySelectorAll('.share-option').forEach(option => {
  option.addEventListener('click', () => {
    ...
    const poem = poemsData[currentPoemId];
    const text = `"${poem.title}" by Black Broken Heart 💔`;
...
pdfBtn.addEventListener('click', () => {
  ...
  content.innerHTML = `
    <div style="padding: 40px; font-family: Georgia, serif;">
      <h1 style="text-align: center; margin-bottom: 10px;">${poem.title}</h1>
      <p style="text-align: center; color: #666; margin-bottom: 30px;">${poem.category}</p>
      <div style="white-space: pre-wrap; line-height: 2; font-size: 14px;">${poem.content}</div>
      <p style="text-align: center; margin-top: 40px; color: #999;">Poetry Universe — Black Broken Heart 💔</p>
    </div>
  `;

  html2pdf().set({
    margin: 10,
    filename: poem.title.replace(/\s+/g, '_') + '.pdf',
...
function updateStatsUI() {
  document.getElementById('statPoems').textContent = stats.poems || 0;
  document.getElementById('statTime').textContent = Math.round((stats.time || 0) / 60) + 'm';
...
const trackGenerators = {
  ...
  bells: (vol) => {
    ...
    fb.gain.value = 0.4;
    osc.connect(gain).connect(delay).connect(ctx.destination);
    delay.connect(fb).connect(delay);
    osc.start();
    return { stop: () => osc.stop(), gain };
  },
  flute: (vol) => {
    ensureAudioCtx();
    const ctx = audioCtx;
    ...
    osc.start();
    vib.start();
    return { stop: () => { osc.stop(); vib.stop(); }, gain };
  },
  choir: (vol) => {
    ensureAudioCtx();
    const ctx = audioCtx;
    ...
    return { stop: () => oscs.forEach(o => o.stop()), gain };
  }
};
...
musicTracks.forEach(track => {
  track.addEventListener('click', () => {
    ...
    musicTracks.forEach(t => t.classList.remove('active'));
    
    const trackType = track.dataset.track;
    const volume = musicVolume.value / 100;
    
    if (trackGenerators[trackType]) {
      currentTrack = trackGenerators[trackType](volume);
...
musicVolume.addEventListener('input', () => {
  const vol = musicVolume.value / 100;
  volumeLabel.textContent = musicVolume.value + '%';
  if (currentTrack && currentTrack.gain) {
    currentTrack.gain.gain.value = vol;
  }
});