...
// This file IS the server. Run: node chat.js
// =====================================================
...
const ADMIN_CONFIG = {
  username: 'admin',
  // Default password: "ChatWaveAdmin2024!" — Change this!
...
const suspendedUsers = new Map(); // username -> { until, reason }
const mutedUsers = new Map(); // username -> { until, reason }
const ipAddressMap = new Map(); // username -> ip
const userStats = new Map(); // username -> { messageCount, joinedAt, lastActive }
const serverLogs = []; // { timestamp, type, message, details }
const announcements = []; // { id, text, timestamp }
let serverSettings = {
  maxMessageLength: 2000,
  slowMode: 0, // seconds between messages per user, 0 = off
...
const slowModeTimers = new Map(); // username -> lastMessageTime
const userWarnings = new Map(); // username -> [{ reason, timestamp, by }]
...
app.get('/', (req, res) => {
  if (serverSettings.maintenanceMode) {
    res.send(`
      <!DOCTYPE html>
      <html><head><title>ChatWave - Maintenance</title>
      <style>body{display:flex;align-items:center;justify-content:center;height:100vh;margin:0;background:#1a1a2e;color:#e0e0e0;font-family:system-ui;text-align:center}
      .container{padding:40px}h1{color:#6C63FF;font-size:48px}p{color:#a0a8c0;font-size:18px}</style></head>
      <body><div class="container"><h1>🔧</h1><h2>Under Maintenance</h2><p>ChatWave is currently undergoing maintenance. Please check back later.</p></div></body></html>
    `);
    return;
  }
...
app.post('/admin/api/login', (req, res) => {
  ...
  if (attempts && attempts.count >= ADMIN_CONFIG.maxLoginAttempts) {
    ...
    if (timeSince < ADMIN_CONFIG.lockoutDuration) {
      ...
      addLog('security', `Locked out login attempt from ${ip}`, { remaining });
  ...
  addLog('security', `Failed admin login attempt from ${ip}`, { attempt: current.count });

  res.status(401).json({
    error: 'Invalid credentials',
    attemptsRemaining: ADMIN_CONFIG.maxLoginAttempts - current.count
  });
});
...
app.get('/admin/api/users', authenticateAdmin, (req, res) => {
  ...
  for (const [name, data] of Object.entries(users)) {
    allUsers[name] = {
      ...
      suspended: suspendedUsers.has(name) ? suspendedUsers.get(name) : null,
      muted: mutedUsers.has(name) ? mutedUsers.get(name) : null,
      stats: userStats.get(name) || { messageCount: 0 },
      warnings: userWarnings.get(name) || []
    };
  }

  // Known offline users from stats
...
app.post('/admin/api/users/:username/kick', authenticateAdmin, (req, res) => {
  ...
  setTimeout(() => {
    if (users[username] && users[username].ws) {
      users[username].ws.close(1000, 'Kicked by admin');
    }
  }, 500);
...
app.post('/admin/api/users/:username/suspend', authenticateAdmin, (req, res) => {
  ...
  const until = duration ? Date.now() + (duration * 60000) : null; // null = permanent
  ...
  if (users[username]) {
    sendToUser(username, {
      ...
      reason: reason || 'Suspended by admin',
      until,
      message: `You have been suspended${duration ? ` for ${duration} minutes` : ' permanently'}: ${reason || 'No reason provided'}`
    });
    setTimeout(() => {
      if (users[username] && users[username].ws) {
        users[username].ws.close(1000, 'Suspended');
      }
    }, 500);
  }
...
app.post('/admin/api/users/:username/mute', authenticateAdmin, (req, res) => {
  ...
  if (users[username]) {
    sendToUser(username, {
      ...
      reason: reason || 'Muted by admin',
      until,
      message: `You have been muted${duration ? ` for ${duration} minutes` : ' permanently'}: ${reason || 'No reason provided'}`
    });
  }
...
app.post('/admin/api/users/:username/warn', authenticateAdmin, (req, res) => {
  ...
  userWarnings.get(username).push({
    reason: reason || 'Warning from admin',
    timestamp: Date.now(),
    by: 'admin'
  });
  ...
  if (users[username]) {
    sendToUser(username, {
      ...
      action: 'warn',
      reason: reason || 'Warning from admin',
      message: `⚠️ Warning: ${reason || 'Please follow the rules.'}`
    });
  }
...
app.post('/admin/api/users/:username/ban-ip', authenticateAdmin, (req, res) => {
  ...
  if (ip) {
    bannedIPs.add(ip);
    addLog('moderation', `Banned IP for user: ${username}`, { ip, reason });
  }
  ...
  if (users[username]) {
    sendToUser(username, {
      type: 'admin_action',
      action: 'ban',
      message: `You have been permanently banned: ${reason || 'No reason provided'}`
    });
    setTimeout(() => {
      if (users[username] && users[username].ws) {
        users[username].ws.close(1000, 'Banned');
      }
    }, 500);
  }
...
app.post('/admin/api/users/:username/color', authenticateAdmin, (req, res) => {
  const { username } = req.params;
...
app.delete('/admin/api/messages/:id', authenticateAdmin, (req, res) => {
  ...
  if (dmKey && directMessages[dmKey]) {
    ...
    if (idx !== -1) {
      ...
      const participants = dmKey.split(':');
      participants.forEach(u => {
        sendToUser(u, { type: 'delete', id, dm: participants.find(p => p !== u) || u });
      });
      found = true;
      addLog('moderation', `Deleted DM message`, { id, from: deleted.from, text: deleted.text?.substring(0, 50) });
    }
  } else {
    // Search channels
    const searchChannels = channel ? [channel] : Object.keys(channels);
    for (const ch of searchChannels) {
      if (channels[ch]) {
        ...
        if (idx !== -1) {
          ...
          found = true;
          addLog('moderation', `Deleted message in #${ch}`, { id, user: deleted.username, text: deleted.text?.substring(0, 50) });
          break;
        }
      }
    }
...
// Purge all messages from a user
app.post('/admin/api/users/:username/purge', authenticateAdmin, (req, res) => {
  ...
  for (const [ch, msgs] of Object.entries(channels)) {
    ...
    purged += before - channels[ch].length;
  ...
  for (const [key, msgs] of Object.entries(directMessages)) {
    ...
    purged += before - directMessages[key].length;
...
app.post('/admin/api/channels/:channel/clear', authenticateAdmin, (req, res) => {
  ...
  const count = channels[channel].length;
  ...
  addLog('moderation', `Cleared channel #${channel}`, { messagesRemoved: count });
...
app.post('/admin/api/channels', authenticateAdmin, (req, res) => {
  ...
  if (!clean || channels[clean]) {
    return res.status(400).json({ error: 'Invalid or existing channel name' });
  }
  channels[clean] = [];
...
app.post('/admin/api/announce', authenticateAdmin, (req, res) => {
  ...
  // Send to all users as system message in all channels
  broadcast({
    type: 'announcement',
    text,
    timestamp: Date.now()
  ...
  // Also post as system message in general
...
// ---- Server Settings ----
app.get('/admin/api/settings', authenticateAdmin, (req, res) => {
...
app.put('/admin/api/settings', authenticateAdmin, (req, res) => {
  ...
  addLog('admin', 'Updated server settings', updates);
  res.json(serverSettings);
});
...
app.get('/admin/api/logs', authenticateAdmin, (req, res) => {
  ...
  res.json(logs.slice(-(parseInt(limit) || 100)));
});
...
// ---- Banned IPs ----
app.get('/admin/api/banned-ips', authenticateAdmin, (req, res) => {
...
// ---- Admin page access (with basic path obfuscation) ----
...
function addLog(type, message, details = null) {
  ...
  if (serverLogs.length > 1000) {
    serverLogs.splice(0, serverLogs.length - 1000);
  }
...
function broadcastUsers() {
  const userList = {};
  for (const [name, data] of Object.entries(users)) {
    userList[name] = { color: data.color };
  }
...
function isUserMuted(username) {
  ...
  const mute = mutedUsers.get(username);
  ...
  ws.on('message', (rawData) => {
    ...
    switch (data.type) {
      ...
      case 'join': {
        ...
        if (serverSettings.maintenanceMode) {
          ws.send(JSON.stringify({ type: 'error', message: 'Server is under maintenance. Please try again later.' }));
          return;
        }

        // Check registration
        if (!serverSettings.registrationOpen && !userStats.has(username)) {
          ws.send(JSON.stringify({ type: 'error', message: 'Registration is currently closed.' }));
          return;
        }
        ...
        if (isUserSuspended(username)) {
          const suspension = suspendedUsers.get(username);
          const timeLeft = suspension.until ? Math.ceil((suspension.until - Date.now()) / 60000) : 'permanently';
          ws.send(JSON.stringify({
            type: 'error',
            message: `You are suspended${typeof timeLeft === 'number' ? ` for ${timeLeft} more minutes` : ' permanently'}: ${suspension.reason}`
          }));
          return;
        }
        ...
        // Reserved names
        if (['admin', 'system', 'moderator', 'server'].includes(username.toLowerCase())) {
          ws.send(JSON.stringify({ type: 'error', message: 'That username is reserved' }));
          return;
        }
        ...
        if (!userStats.has(username)) {
          userStats.set(username, { messageCount: 0, joinedAt: Date.now(), lastActive: Date.now() });
        } else {
          userStats.get(username).lastActive = Date.now();
        }
        ...
        if (isUserMuted(username)) {
          ...
          ws.send(JSON.stringify({
            type: 'admin_action',
            action: 'mute_notice',
            message: `You are currently muted: ${mute.reason}`
          }));
        }

        // Send channel history for general
        ws.send(JSON.stringify({
          type: 'history',
          channel: 'general',
          messages: channels.general.slice(-100)
        }));

        // Send existing channels
        for (const ch of Object.keys(channels)) {
          if (!['general', 'random', 'tech'].includes(ch)) {
            ws.send(JSON.stringify({ type: 'channel_created', channel: ch }));
          }
        }
      ...
      case 'message': {
        ...
        // Check muted
        if (isUserMuted(currentUsername)) {
          ws.send(JSON.stringify({ type: 'error', message: 'You are muted and cannot send messages.' }));
          return;
        }

        // Check slow mode
        if (serverSettings.slowMode > 0) {
          const lastMsg = slowModeTimers.get(currentUsername);
          if (lastMsg && (Date.now() - lastMsg) < serverSettings.slowMode * 1000) {
            const wait = Math.ceil((serverSettings.slowMode * 1000 - (Date.now() - lastMsg)) / 1000);
            ws.send(JSON.stringify({ type: 'error', message: `Slow mode: wait ${wait}s before sending another message.` }));
            return;
          }
        }

        // Check message length
        if (data.text && data.text.length > serverSettings.maxMessageLength) {
          ws.send(JSON.stringify({ type: 'error', message: `Message too long. Max ${serverSettings.maxMessageLength} characters.` }));
          return;
        }
        ...
        // Apply word filter
        const filteredText = checkWordFilter(data.text || '');
        ...
        const msg = {
          ...
          edited: false,
          timestamp: Date.now(),
          color: users[currentUsername].color
        };

        channels[channel].push(msg);
        if (channels[channel].length > 500) {
          channels[channel] = channels[channel].slice(-500);
        }
      ...
      case 'dm': {
        ...
        const dmKey = getDMKey(currentUsername, toUser);
        ...
        if (directMessages[dmKey].length > 500) {
          directMessages[dmKey] = directMessages[dmKey].slice(-500);
        }
      ...
      case 'get_history': {
        ...
        ws.send(JSON.stringify({
          type: 'history',
          channel: ch,
          messages: channels[ch].slice(-100)
        }));
        break;
      }
      ...
      case 'get_dm_history': {
        ...
        ws.send(JSON.stringify({
          type: 'history',
          dm: otherUser,
          messages: directMessages[dmKey].slice(-100)
        }));
        break;
      }
      ...
      case 'stop_typing': {
        ...
        if (data.dm) {
          sendToUser(data.dm, stopData);
        } else {
          broadcast(stopData, ws);
        }
        break;
      }
      ...
      case 'edit': {
        ...
        if (data.dm) {
          ...
          if (directMessages[dmKey]) {
            const msg = directMessages[dmKey].find(m => m.id === data.id);
            if (msg && (msg.from === currentUsername || msg.username === currentUsername)) {
              msg.text = checkWordFilter(data.text);
              msg.edited = true;
              found = true;
          ...
          if (channels[data.channel]) {
            const msg = channels[data.channel].find(m => m.id === data.id);
            if (msg && msg.username === currentUsername) {
              msg.text = checkWordFilter(data.text);
              msg.edited = true;
              found = true;
              broadcast({ type: 'edit', id: data.id, text: msg.text, channel: data.channel });
            }
          }
        }
        ...
        if (!found) {
          for (const [ch, msgs] of Object.entries(channels)) {
            ...
            if (msg && msg.username === currentUsername) {
              msg.text = checkWordFilter(data.text);
              msg.edited = true;
              broadcast({ type: 'edit', id: data.id, text: msg.text, channel: ch });
              break;
            }
          }
        }
      ...
      case 'delete': {
        ...
        if (data.dm) {
          const dmKey = getDMKey(currentUsername, data.dm);
          if (directMessages[dmKey]) {
            const idx = directMessages[dmKey].findIndex(m => m.id === data.id && (m.from === currentUsername || m.username === currentUsername));
            if (idx !== -1) {
          ...
          if (channels[data.channel]) {
            ...
            if (idx !== -1) {
              channels[data.channel].splice(idx, 1);
              broadcast({ type: 'delete', id: data.id, channel: data.channel });
            }
          }
        } else {
          for (const [ch, msgs] of Object.entries(channels)) {
            ...
            if (idx !== -1) {
              msgs.splice(idx, 1);
              broadcast({ type: 'delete', id: data.id, channel: ch });
              break;
            }
          }
        }
      ...
      case 'reaction': {
        ...
        if (data.dm) {
          const dmKey = getDMKey(currentUsername, data.dm);
          if (directMessages[dmKey]) {
            targetMsg = directMessages[dmKey].find(m => m.id === data.id);
            broadcastTarget = { dm: data.dm };
          }
        } else if (data.channel) {
          if (channels[data.channel]) {
            targetMsg = channels[data.channel].find(m => m.id === data.id);
            broadcastTarget = { channel: data.channel };
          }
        } else {
          for (const [ch, msgs] of Object.entries(channels)) {
            targetMsg = msgs.find(m => m.id === data.id);
            if (targetMsg) {
              broadcastTarget = { channel: ch };
              break;
            }
          }
        }
        ...
        if (targetMsg) {
          ...
          const idx = targetMsg.reactions[emoji].indexOf(currentUsername);
          if (idx !== -1) {
            ...
            targetMsg.reactions[emoji].push(currentUsername);
          }
          ...
          if (broadcastTarget.dm) {
            sendToUser(currentUsername, reactionUpdate);
            sendToUser(broadcastTarget.dm, reactionUpdate);
          } else {
            broadcast(reactionUpdate);
          }
        }
        break;
      }
  ...
  ws.on('close', () => {
    if (currentUsername && users[currentUsername]) {
      ...
      const leaveMsg = {
        ...
        channel: 'general',
        username: 'System',
        text: `${currentUsername} has left the chat`,
        system: true,
        timestamp: Date.now(),
        color: '#888'
      };
...
server.listen(PORT, () => {
  ...
  console.log('');
  console.log('╔══════════════════════════════════════════╗');
  console.log('║          🌊 ChatWave Server 🌊           ║');
  console.log('╠══════════════════════════════════════════╣');
  console.log(`║  🚀 Chat:  http://localhost:${PORT}          ║`);
  console.log(`║  🔐 Admin: http://localhost:${PORT}/admin     ║`);
  console.log('║  📱 Open multiple tabs to test           ║');
  console.log('║  ⌨️  Press Ctrl+C to stop                 ║');
  console.log('╠══════════════════════════════════════════╣');
  console.log('║  Admin Login:                            ║');
  console.log('║  Username: admin                         ║');
  console.log('║  Password: ChatWaveAdmin2024!            ║');
  console.log('╚══════════════════════════════════════════╝');
  console.log('');
});
...
app.post('/admin/api/users/:username/kick', authenticateAdmin, (req, res) => {
  ...
  setTimeout(() => { if (users[username] && users[username].ws) users[username].ws.close(1000, 'Kicked by admin'); }, 500);
  addLog('moderation', `Kicked user: ${username}`, { reason });
  broadcastSystemMessage(`${username} has been removed from the chat.`);
  res.json({ success: true });
});
...
app.post('/admin/api/users/:username/suspend', authenticateAdmin, (req, res) => {
  ...
  if (users[username]) {
    sendToUser(username, { type: 'admin_action', action: 'suspend', reason: reason || 'Suspended by admin', until, message: `You have been suspended${duration ? ` for ${duration} minutes` : ' permanently'}: ${reason || 'No reason provided'}` });
    setTimeout(() => { if (users[username] && users[username].ws) users[username].ws.close(1000, 'Suspended'); }, 500);
  }
  addLog('moderation', `Suspended user: ${username}`, { duration, reason, until });
  res.json({ success: true });
});
...
app.post('/admin/api/users/:username/mute', authenticateAdmin, (req, res) => {
  ...
  if (users[username]) {
    sendToUser(username, { type: 'admin_action', action: 'mute', reason: reason || 'Muted by admin', until, message: `You have been muted${duration ? ` for ${duration} minutes` : ' permanently'}: ${reason || 'No reason provided'}` });
  }
  addLog('moderation', `Muted user: ${username}`, { duration, reason });
  res.json({ success: true });
});
...
app.post('/admin/api/users/:username/unmute', authenticateAdmin, (req, res) => {
  ...
  if (users[req.params.username]) {
    sendToUser(req.params.username, { type: 'admin_action', action: 'unmute', message: 'You have been unmuted.' });
  }
  addLog('moderation', `Unmuted user: ${req.params.username}`);
  res.json({ success: true });
});
...
app.post('/admin/api/users/:username/warn', authenticateAdmin, (req, res) => {
  ...
  if (users[username]) {
    sendToUser(username, { type: 'admin_action', action: 'warn', reason: reason || 'Warning from admin', message: `⚠️ Warning: ${reason || 'Please follow the rules.'}` });
  }
  addLog('moderation', `Warned user: ${username}`, { reason });
  res.json({ success: true, totalWarnings: userWarnings.get(username).length });
});
...
app.post('/admin/api/users/:username/ban-ip', authenticateAdmin, (req, res) => {
  ...
  if (users[username]) {
    ...
    setTimeout(() => { if (users[username] && users[username].ws) users[username].ws.close(1000, 'Banned'); }, 500);
  }
  res.json({ success: true, ip });
});
...
app.post('/admin/api/users/:username/purge', authenticateAdmin, (req, res) => {
  ...
  for (const [key, msgs] of Object.entries(directMessages)) {
    ...
    purged += toDelete.length;
    const participants = key.split(':');
    toDelete.forEach(id => { participants.forEach(u => { sendToUser(u, { type: 'delete', id, dm: participants.find(p => p !== u) || u }); }); });
  }
  addLog('moderation', `Purged ${purged} messages from ${username}`);
  res.json({ success: true, purged });
});
...
app.get('/admin/api/dms', authenticateAdmin, (req, res) => {
  const dmSummary = {};
  for (const [key, msgs] of Object.entries(directMessages)) {
    dmSummary[key] = { participants: key.split(':'), messageCount: msgs.length, lastMessage: msgs.length > 0 ? msgs[msgs.length - 1] : null };
  }
  res.json(dmSummary);
});
...
app.delete('/admin/api/messages/:id', authenticateAdmin, (req, res) => {
  ...
  if (dmKey && directMessages[dmKey]) {
    ...
    if (idx !== -1) {
      const deleted = directMessages[dmKey].splice(idx, 1)[0];
      const participants = dmKey.split(':');
      participants.forEach(u => { sendToUser(u, { type: 'delete', id, dm: participants.find(p => p !== u) || u }); });
      found = true;
      addLog('moderation', `Deleted DM message`, { id, from: deleted.from, text: deleted.text?.substring(0, 50) });
    }
  } else {
...
app.delete('/admin/api/channels/:channel', authenticateAdmin, (req, res) => {
  const { channel } = req.params;
  if (['general'].includes(channel)) return res.status(400).json({ error: 'Cannot delete the general channel' });
  if (!channels[channel]) return res.status(404).json({ error: 'Channel not found' });
  delete channels[channel];
  broadcast({ type: 'channel_deleted', channel });
  addLog('moderation', `Deleted channel #${channel}`);
...
app.get('/admin/api/channels', authenticateAdmin, (req, res) => {
  const chList = {};
  for (const [name, msgs] of Object.entries(channels)) {
    chList[name] = { name, messageCount: msgs.length, lastActivity: msgs.length > 0 ? msgs[msgs.length - 1].timestamp : null };
  }
  res.json(chList);
});
...
app.delete('/admin/api/logs', authenticateAdmin, (req, res) => { serverLogs.length = 0; res.json({ success: true }); });
...
function broadcastSystemMessage(text, channel = 'general') {
  const msg = { id: uuidv4(), type: 'message', channel, username: 'System', text, system: true, timestamp: Date.now(), color: '#888' };
  if (channels[channel]) channels[channel].push(msg);
  broadcast(msg);
}
...
function broadcast(data, excludeWs = null) {
  const msg = JSON.stringify(data);
  wss.clients.forEach(client => {
    if (client.readyState === 1 && client !== excludeWs && !client.__isAdmin) client.send(msg);
  });
}
...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', (rawData) => {
    ...
    switch (data.type) {
      case 'join': {
        const username = data.username.trim();
        if (!username || username.length > 20) { ws.send(JSON.stringify({ type: 'error', message: 'Invalid username' })); return; }
        if (serverSettings.maintenanceMode) { ws.send(JSON.stringify({ type: 'error', message: 'Server is under maintenance. Please try again later.' })); return; }
        if (!serverSettings.registrationOpen && !userStats.has(username)) { ws.send(JSON.stringify({ type: 'error', message: 'Registration is currently closed.' })); return; }
        if (isUserSuspended(username)) {
          const s = suspendedUsers.get(username);
          const timeLeft = s.until ? Math.ceil((s.until - Date.now()) / 60000) : 'permanently';
          ws.send(JSON.stringify({ type: 'error', message: `You are suspended${typeof timeLeft === 'number' ? ` for ${timeLeft} more minutes` : ' permanently'}: ${s.reason}` }));
          return;
        }
        if (users[username]) { ws.send(JSON.stringify({ type: 'error', message: 'Username already taken' })); return; }
        if (['admin', 'system', 'moderator', 'server'].includes(username.toLowerCase())) { ws.send(JSON.stringify({ type: 'error', message: 'That username is reserved' })); return; }
        ...
        if (isUserMuted(username)) { const m = mutedUsers.get(username); ws.send(JSON.stringify({ type: 'admin_action', action: 'mute_notice', message: `You are currently muted: ${m.reason}` })); }
        ws.send(JSON.stringify({ type: 'history', channel: 'general', messages: channels.general.slice(-100) }));
        for (const ch of Object.keys(channels)) { if (!['general', 'random', 'tech'].includes(ch)) ws.send(JSON.stringify({ type: 'channel_created', channel: ch })); }
      ...
      case 'message': {
        if (!currentUsername) return;
        if (isUserMuted(currentUsername)) { ws.send(JSON.stringify({ type: 'error', message: 'You are muted and cannot send messages.' })); return; }
        if (serverSettings.slowMode > 0) {
          const lastMsg = slowModeTimers.get(currentUsername);
          if (lastMsg && (Date.now() - lastMsg) < serverSettings.slowMode * 1000) {
            const wait = Math.ceil((serverSettings.slowMode * 1000 - (Date.now() - lastMsg)) / 1000);
        ...
        if (data.text && data.text.length > serverSettings.maxMessageLength) { ws.send(JSON.stringify({ type: 'error', message: `Message too long. Max ${serverSettings.maxMessageLength} characters.` })); return; }

        const channel = data.channel || 'general';
        if (!channels[channel]) channels[channel] = [];
        const msg = {
          id: uuidv4(), type: 'message', channel, username: currentUsername, text: checkWordFilter(data.text || ''),
          file: data.file || null, replyTo: data.replyTo || null, replyText: data.replyText || null, replyUser: data.replyUser || null,
      ...
      case 'dm': {
        ...
        if (isUserMuted(currentUsername)) { ws.send(JSON.stringify({ type: 'error', message: 'You are muted and cannot send messages.' })); return; }
        const toUser = data.to;
        if (!toUser || !users[toUser]) { ws.send(JSON.stringify({ type: 'error', message: 'User not found' })); return; }
        const dmKey = getDMKey(currentUsername, toUser);
        if (!directMessages[dmKey]) directMessages[dmKey] = [];
        const msg = {
          id: uuidv4(), type: 'dm', from: currentUsername, to: toUser, text: checkWordFilter(data.text || ''),
          file: data.file || null, replyTo: data.replyTo || null, replyText: data.replyText || null, replyUser: data.replyUser || null,
          reactions: {}, edited: false, timestamp: Date.now(), color: users[currentUsername].color
        };
        directMessages[dmKey].push(msg);
      ...
      case 'create_channel': {
        if (!currentUsername) return;
        const channelName = data.channel.toLowerCase().replace(/[^a-z0-9\-_]/g, '');
        if (!channelName || channelName.length > 20) { ws.send(JSON.stringify({ type: 'error', message: 'Invalid channel name' })); return; }
        if (channels[channelName]) { ws.send(JSON.stringify({ type: 'error', message: 'Channel already exists' })); return; }
        channels[channelName] = [];
        broadcast({ type: 'channel_created', channel: channelName });
        addLog('channel', `${currentUsername} created channel #${channelName}`);
        break;
      ...
      case 'typing': {
        ...
        if (data.dm) sendToUser(data.dm, td); else broadcast(td, ws);
        break;
      }
      ...
      case 'stop_typing': {
        ...
        if (data.dm) sendToUser(data.dm, sd); else broadcast(sd, ws);
        break;
      }
      ...
      case 'edit': {
        ...
        if (data.dm) {
          ...
          if (directMessages[dmKey]) {
            const msg = directMessages[dmKey].find(m => m.id === data.id);
            if (msg && (msg.from === currentUsername)) {
        ...
        if (!found) {
          for (const [ch, msgs] of Object.entries(channels)) {
            const msg = msgs.find(m => m.id === data.id);
            if (msg && msg.username === currentUsername) {
              msg.text = checkWordFilter(data.text); msg.edited = true;
              broadcast({ type: 'edit', id: data.id, text: msg.text, channel: ch }); break;
            }
          }
        }
        break;
      ...
      case 'reaction': {
        ...
        if (targetMsg) {
          ...
          if (idx !== -1) { targetMsg.reactions[data.emoji].splice(idx, 1); if (targetMsg.reactions[data.emoji].length === 0) delete targetMsg.reactions[data.emoji]; }
          ...
          if (broadcastTarget.dm) { sendToUser(currentUsername, update); sendToUser(broadcastTarget.dm, update); }
